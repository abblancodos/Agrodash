...
<script lang="ts">
  ...
  // nodeTypes object — required by SvelteFlow for custom node rendering
  ...
  // ── Reactive nodes/edges arrays for active pipeline ──────────────────────
  // Using $state arrays so SvelteFlow reacts to changes
...
<div class="config-tab">
        ...
        {#if activePl === i && $canAdmin}
          <div class="pl-tab-actions">
            ...
            <button class="tact" onclick={() => movePipeline(i, 1)}
              disabled={i === (draft?.pipelines.length ?? 1) - 1}>↓</button>
  ...
  {#if !draft || draft.pipelines.length === 0}
    <div class="empty-canvas">No hay pipelines. Hacé click en <strong>+ pipeline</strong> para crear uno.</div>
  {:else}
    {@const pl = draft.pipelines[activePl]}
    ...
    {#if $canAdmin}
      <div class="pl-name-row">
        ...
        <input id="pl-interval" class="pl-interval-input" type="number" min="5"
          ...
          <div class="side-panel">
            <div class="sp-head">
              ...
              <div class="sp-actions">
                {#if $canAdmin}
                  <button class="sp-del" onclick={() => removeNode(panelNode!.plIdx, nd.id)} title="eliminar">🗑</button>
                {/if}
            ...
            <div class="sp-body">
              ...
              {#if nd.type === 'postgres_sensor'}
                <div class="sp-field"><div class="sp-section-label">sensores</div>
              ...
              {:else if nd.type === 'kalman'}
                {#each [['Q','varianza proceso'],['R','varianza medición'],['P0','cov. inicial'],['convergence_threshold','umbral conv.']] as [f, h] (f)}
                  <div class="sp-field">
              ...
              {:else if nd.type === 'moving_avg'}
                <div class="sp-field">
                  ...
                  <input id="ma-win" type="number" value={nd.window_n} oninput={(e) => updateNodeField('window_n', parseInt((e.target as HTMLInputElement).value))} />
                </div>
                <div class="sp-field">
                  <label for="ma-warm">warmup_samples</label>
                  <input id="ma-warm" type="number" value={nd.warmup_samples} oninput={(e) => updateNodeField('warmup_samples', parseInt((e.target as HTMLInputElement).value))} />
                </div>
              ...
              {:else if nd.type === 'mqtt_actuator'}
                ...
                <div class="sp-field">
                  ...
                  <input id="mqtt-topic" class="mono" value={nd.topic ?? ''}
                    oninput={(e) => updateNodeField('topic', (e.target as HTMLInputElement).value)} placeholder="relay/control" />
                </div>
                <div class="sp-field-row">
                  <div class="sp-field">
...
<style>
  ...
  .add-btn { padding: calc(3px * var(--font-scale)) calc(8px * var(--font-scale)); border: 0.5px solid var(--border-default); border-radius: 4px; background: none; cursor: pointer; font-size: calc(10px * var(--font-scale)); font-family: 'DM Mono', monospace; white-space: nowrap; transition: opacity .1s; }
  .add-btn:hover { opacity: 0.75; background: var(--interactive-hover); }
  ...
  .pl-name-row { display: flex; align-items: center; gap: 8px; padding: calc(6px * var(--font-scale)) 0; }
  ...
  .pl-interval-input { width: 60px; border: none; border-bottom: 0.5px solid var(--border-default); background: none; font-size: calc(13px * var(--font-scale)); color: var(--text-primary); outline: none; font-family: 'DM Mono', monospace; padding: 2px 4px; }
  ...
  .sp-section-label { font-size: calc(11px * var(--font-scale)); color: var(--text-muted); font-weight: 500; margin-bottom: 4px; }
