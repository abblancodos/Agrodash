...
<script lang="ts">
  ...
  // nodeData es el state.data del nodo UPSTREAM conectado a este Logger.
  // Se actualiza cada ciclo del agente (vía pollPipelineStates en +page.svelte).
  const currentValue = $derived.by(() => {
    if (!nodeData) return '—';
    switch (nodeType) {
      case 'kalman': {
        const est = nodeData.x?.[0];
        const raw = nodeData.raw?.[0]; // no existe en kalman state, viene de readings
        return est != null ? est.toFixed(4) : '—';
      }
      case 'ewma': case 'lowpass': case 'moving_avg':
        return (nodeData.y?.[0] ?? nodeData.x?.[0])?.toFixed(4) ?? '—';
      case 'mahalanobis': {
        // last_d = distancia de Mahalanobis del último ciclo
        // hyst = decisión actual (on/off/hold)
