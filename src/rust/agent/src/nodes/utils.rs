...
pub struct WeightedMeanNode {
    id: String,
    weights: Vec<f64>,
}
impl WeightedMeanNode {
    pub fn new(id: String, weights: Vec<f64>) -> Self {
        Self { id, weights }
    }
}
...
impl NodeInstance for WeightedMeanNode {
    async fn execute(
        ...
        let vecs: Vec<Vec<f64>> = inputs
            .iter()
            .map(|s| expect_vector(s, &self.id))
            .collect::<Result<_>>()?;
        let dim = vecs[0].len();
...
impl SelectNode {
    pub fn new(id: String, indices: Vec<usize>) -> Self {
        Self { id, indices }
    }
}
...
impl NodeInstance for SelectNode {
    async fn execute(
        ...
        let v = expect_vector(
            inputs
                .first()
                .ok_or_else(|| anyhow::anyhow!("Select sin entrada"))?,
            &self.id,
        )?;
...
pub struct LinearScaleNode {
    id: String,
    a: VecOrScalar,
    b: VecOrScalar,
}
impl LinearScaleNode {
    pub fn new(id: String, a: VecOrScalar, b: VecOrScalar) -> Self {
        Self { id, a, b }
    }
}
...
impl NodeInstance for LinearScaleNode {
    async fn execute(
        ...
        let x = expect_vector(
            inputs
                .first()
                .ok_or_else(|| anyhow::anyhow!("LinearScale sin entrada"))?,
            &self.id,
        )?;
        let n = x.len();
