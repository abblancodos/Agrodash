...
macro_rules! test_rt {
    ($nombre:ident, $tipo:ty, $valor:expr) => {
        ...
        fn $nombre() {
            let original = $valor;
            let json = serde_json::to_string(&original).expect("serialize falló");
            let parsed: $tipo =
                serde_json::from_str(&json).expect("deserialize falló — ¿falta #[serde(default)]?");
            let json2 = serde_json::to_string(&parsed).expect("re-serialize falló");
            assert_eq!(
                json, json2,
...
test_rt!(
    ...
    WatchdogConfig {
        ...
        mode: WatchdogMode::Ugly {
            notify_message: Some("verificar presión manual".into())
        },
...
test_rt!(
    reduction_component_rt,
    Reduction,
    Reduction::Component { index: 2 }
);
test_rt!(reduction_weighted_rt, Reduction, Reduction::WeightedByP);
test_rt!(
    reduction_count_below_rt,
    Reduction,
...
fn hysteresis_antigua_sin_trend_funciona() {
    ...
    let cfg: HysteresisConfig = serde_json::from_str(json).expect(
        "HysteresisConfig antigua sin 'trend' no deserializa — \
...
fn edge_antiguo_sin_kind_es_data() {
    ...
    let edge: EdgeConfig =
        serde_json::from_str(json).expect("EdgeConfig sin 'kind' no deserializa");
    assert_eq!(
        edge.kind,
        EdgeKind::Data,
...
fn sprt_antiguo_sin_trend_funciona() {
    ...
    let _cfg: SprtConfig =
        serde_json::from_str(json).expect("SprtConfig antigua sin 'trend' no deserializa");
}
...
fn watchdog_antiguo_sin_timeout_ni_retries_usa_defaults() {
    ...
    let cfg: WatchdogConfig = serde_json::from_str(json).expect(
        "WatchdogConfig sin timeout/retries no deserializa — \
                 verificar #[serde(default)] en action_timeout_secs y max_retries",
    );
    assert_eq!(
        cfg.action_timeout_secs, 30,
        "default de action_timeout_secs debe ser 30"
    );
